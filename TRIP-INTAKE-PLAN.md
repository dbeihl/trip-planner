...
- **Immutable gates** (built with Phase 2): auto-merge is only as trustworthy as the gates a PR cannot rewrite, so `.github/workflows/guard.yml` runs on `pull_request_target` — GitHub always executes the *base branch's* copy, so a PR editing the guard (or any gate) is judged by `main`'s version, never its own. It fails any PR touching protected paths (workflows, tests, schema, the validator, dependency manifests) until the repo owner approves; PRs touching none of those — exactly the shape of a generated trip — pass instantly and stay auto-mergeable. One-time setup that gives it teeth, in repo Settings: branch protection on `main` requiring the `test`, `smoke`, and `Guard` checks, and General → allow auto-merge. Residual truth: the repo owner can always change anything on `main` directly — "immutable" means no PR can weaken the gates and then be judged by the weakened version.
...
2. **The spine, no AI** — **built**: `POST /api/trips` + `GET /api/trips/:id` on the Worker (`worker/src/intake.js`: allowlist via `TRIP_CREATORS`, quotas per the Decisions, `trip_request` D1 table via migration 0004, ledger issue + `repository_dispatch` via a fine-grained `GITHUB_INTAKE_TOKEN`), the `Trip intake` Action stub (comments the brief on the issue, opens a draft PR carrying `research/<slug>.request.json`), and the `Guard` workflow (see Immutable gates). To turn on: set the two Worker bindings (`GITHUB_INTAKE_TOKEN` secret, `TRIP_CREATORS` var), run `npm run migrate:remote`, redeploy, and flip the one-time branch-protection settings.
